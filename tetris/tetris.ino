...
void setup() {
  ...
  while(1) {
    if (digitalRead( buttonPinBUTTON )) {
      while (digitalRead( buttonPinBUTTON ));
      break;
    }
  }
  TV.delay(500);
...
#define PRES (prev & RESBIT)
  if (digitalRead( buttonPinRIGHT )) {
    ...
    if (!digitalRead( buttonPinRIGHT ) && !digitalRead( buttonPinLEFT )) {
      prev &= ~PAUSBIT;
    }
  }
  
  if (game.isOver && digitalRead( buttonPinBUTTON ))
    prev |= RESBIT;
  else if (PRES) {
  ...
  if ( 1 == 2 && analogRead( joyupdown ) < 500) {
    if (!PFIRE) {
  ...
  if (analogRead( joyleftright ) < 500 && !PPAUSE) {
    if (!PLEFT) {
  ...
  if ( analogRead( joyleftright ) > 600  && !PPAUSE) {
    if (!PRIGHT) {
