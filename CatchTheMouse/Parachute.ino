...
boolean getInput()
{
  if ( gameActive && analogRead( joyleftright ) < 500 )
    moveBoat( false );
  else if ( gameActive && analogRead( joyleftright ) > 600 )
    moveBoat( true );
  else if ( menuIsActive && analogRead( joyupdown ) < 400 && selectedGame == eGameB )
    updateMenu( eGameA );
  else if ( menuIsActive && analogRead( joyupdown ) > 800 && selectedGame == eGameA )
    updateMenu( eGameB );
  else if ( menuIsActive && digitalRead( buttonPinBUTTON ) )
    gameStart();
  else if ( !menuIsActive && !gameActive && digitalRead( buttonPinBUTTON ) )
    displayMenu();
 }
